...
def surroundings(listIn, pos):
    numCords = []
    if listIn[pos[0]][pos[1] + 1] in numbers:
        numCords.append(oc['r'])
    if listIn[pos[0]][pos[1] - 1] in numbers:
        numCords.append(oc['l'])
    if listIn[pos[0] - 1][pos[1]] in numbers:
        numCords.append(oc['t'])
    else:
        if listIn[pos[0] - 1][pos[1] + 1] in numbers:
            numCords.append(oc['tr'])
        if listIn[pos[0] - 1][pos[1] - 1] in numbers:
            numCords.append(oc['tl'])
    if listIn[pos[0] + 1][pos[1]] in numbers:
        numCords.append(oc['b'])
    else:
        if listIn[pos[0] + 1][pos[1] + 1] in numbers:
            numCords.append(oc['br'])
        if listIn[pos[0] + 1][pos[1] - 1] in numbers:
            numCords.append(oc['bl'])
    return numCords


def fetchNumber(listIn, pos):
    finalNum = f'{listIn[pos[0]][pos[1]]}'
    # going left
    gl = 0
    while True:
        gl -= 1
        if (pos[1] + gl) >= 0:
            if listIn[pos[0]][pos[1] + gl] in numbers:
                finalNum = f'{listIn[pos[0]][pos[1] + gl]}{finalNum}'
            else:
                break
        else:
            break
    ...
    while True:
        gr += 1
        if (pos[1] + gr) < len(listIn[pos[0]]):
            if listIn[pos[0]][pos[1] + gr] in numbers:
                finalNum = f'{finalNum}{listIn[pos[0]][pos[1] + gr]}'
            else:
                break
        else:
            break
...
lineList = puzzleIn.rsplit('\n')
addNumbers = []
addGears = []
for l in range(len(lineList)):
    if l <= (len(lineList) - 1):
        for i in range(len(lineList[l])):
            if lineList[l][i] in symbols:
                hasNumber = surroundings(lineList, [l, i])
                if hasNumber:
                    for n in hasNumber:
                        addNumbers.append(int(fetchNumber(lineList, [l + n[0], i + n[1]])))
                if len(hasNumber) == 2:
                    gearRatio = 1
                    for n in hasNumber:
                        gearRatio *= int(fetchNumber(lineList, [l + n[0], i + n[1]]))
                    addGears.append(gearRatio)
